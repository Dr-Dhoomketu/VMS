...
function QrCodeImage({ token }: { token: string }) {
  ...
  useEffect(() => {
    if (token) {
      QRCode.toDataURL(`${window.location.origin}/scan/${token}`, { width: 220, margin: 2, color: { dark: '#0A1F44', light: '#FFFFFF' } })
...
interface DivertRequest { _id: string; from?: { name: string }; to?: { name: string }; reason?: string; status: string; at: string; }
interface Visit {
  _id: string; purpose: string; scheduledTime?: string; createdAt: string; updatedAt: string; status: string;
  ...
  meetWith?: { _id?: string; name: string; email?: string; department?: string };
  meetWithDept?: { _id?: string; name: string };
  divertHistory?: DivertHistory[];
  divertRequests?: DivertRequest[];
}
...
function visitDate(v: Visit): Date { return v.scheduledTime ? new Date(v.scheduledTime) : new Date(v.updatedAt); }
function isPast(v: Visit) { const d = visitDate(v); const t = new Date(); d.setHours(0,0,0,0); t.setHours(0,0,0,0); return d < t; }
function fmtDate(d: Date) { return d.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }); }

// ─── Divert Request Modal (request-based, for Pending visits only) ────────────
interface DivertReqModalProps { visit: Visit; myDeptId: string; onClose: () => void; onSent: () => void; }
function DivertRequestModal({ visit, myDeptId, onClose, onSent }: DivertReqModalProps) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API_URL}/api/v1/users/employees?department=${myDeptId}`)
      .then(r => r.json())
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/v1/visits/${visit._id}/divert-request`, {
        method: 'POST',
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,31,68,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 440, boxShadow: '0 20px 60px rgba(10,31,68,0.2)', overflow: 'hidden' }}>
        <div style={{ background: '#0A1F44', padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: '0.5rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.5)', fontWeight: 800 }}>Request Divert</div>
            <div style={{ fontSize: '1rem', fontWeight: 900, color: '#fff' }}>{visit.visitor?.name}</div>
            <div style={{ fontSize: '0.6rem', color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>The other employee must accept before the visit is transferred</div>
          </div>
        ...
        <div style={{ padding: '20px 24px' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#16a34a' }}>Divert request sent!</div>
              <div style={{ fontSize: '0.7rem', color: '#6B7FA3', marginTop: 4 }}>Waiting for the other employee to accept.</div>
            </div>
          ) : (
            <>
              <div style={{ fontSize: '0.6rem', fontWeight: 700, color: '#6B7FA3', textTransform: 'uppercase', letterSpacing: '0.15em', marginBottom: 8 }}>Choose a colleague in your department</div>
              <div style={{ position: 'relative', marginBottom: 10 }}>
                ...
                <input type="text" placeholder="Search colleague…" value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  onFocus={e => e.target.style.borderColor = '#2F5DAA'} onBlur={e => e.target.style.borderColor = '#E2E8F0'}
                />
              </div>
              <div style={{ maxHeight: 200, overflowY: 'auto', border: '1px solid #E2E8F0', borderRadius: 10, marginBottom: 14 }}>
                ...
                {!loading && filtered.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: '#A0AEC0', fontSize: '0.75rem' }}>No colleagues found</div>}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={handleSend} disabled={!selectedEmp || submitting}
                  style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: selectedEmp ? '#2F5DAA' : '#E2E8F0', fontSize: '0.72rem', fontWeight: 900, color: selectedEmp ? '#fff' : '#94A3B8', cursor: selectedEmp ? 'pointer' : 'not-allowed', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                  {submitting ? 'Sending…' : '↪ Send Divert Request'}
                </button>
              </div>
            </>
          )}
...
function VisitorPass({ visit, past }: { visit: Visit; past?: boolean }) {
  ...
  return (
    <div style={{ background: past ? 'linear-gradient(135deg,#f8fafc,#f1f5f9)' : 'linear-gradient(135deg,#ffffff,#f8fafc 50%,#ffffff)', border: '1px solid #E2E8F0', borderRadius: 24, overflow: 'hidden', boxShadow: past ? 'none' : '0 4px 24px rgba(10,31,68,0.08)', opacity: past ? 0.8 : 1 }}>
      <div style={{ height: 3, background: past ? '#CBD5E1' : 'linear-gradient(90deg,#0A1F44,rgba(47,93,170,0.3))' }}/>
      ...
      <div style={{ padding: 24, display: 'flex', gap: 20, background: past ? '#f8fafc' : '#fff' }}>
        <div style={{ flexShrink: 0 }}>
          <div style={{ width: 90, height: 110, borderRadius: 12, border: '1px solid #E2E8F0', overflow: 'hidden', background: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {visit.visitor?.imageUrl
              ...
              : <div style={{ textAlign: 'center', padding: 8 }}><div style={{ fontSize: '1.8rem', marginBottom: 4 }}>👤</div><div style={{ fontSize: '0.45rem', color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em' }}>No Photo</div></div>
            }
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '1.3rem', fontWeight: 900, color: '#0A1F44', textTransform: 'uppercase', lineHeight: 1.1, marginBottom: 4 }}>{visit.visitor?.name || '—'}</div>
          ...
          {(visit.fromTime || visit.duration) && (
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              {visit.fromTime && <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 8, padding: '5px 10px', flex: 1 }}><div style={{ fontSize: '0.45rem', color: '#6B7FA3', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 800 }}>From</div><div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#0A1F44' }}>{fmt12(visit.fromTime)}</div></div>}
              {visit.duration && <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 8, padding: '5px 10px', flex: 1 }}><div style={{ fontSize: '0.45rem', color: '#6B7FA3', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 800 }}>Duration</div><div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#0A1F44' }}>{visit.duration}</div></div>}
            </div>
          )}
          {visit.divertHistory && visit.divertHistory.length > 0 && (
            <div style={{ marginTop: 12, padding: '8px 10px', background: 'rgba(124,58,237,0.04)', border: '1px solid rgba(124,58,237,0.15)', borderRadius: 8 }}>
              <div style={{ fontSize: '0.45rem', color: '#7C3AED', textTransform: 'uppercase', letterSpacing: '0.2em', fontWeight: 800, marginBottom: 4 }}>Divert History</div>
              {visit.divertHistory.map((d, i) => (
                <div key={i} style={{ fontSize: '0.62rem', color: '#6B7FA3', marginBottom: 2 }}>
                  {d.from?.name || 'Dept'} → <strong style={{ color: '#0A1F44' }}>{d.to?.name}</strong>{d.reason ? ` · ${d.reason}` : ''}
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          <div style={{ width: 100, height: 100, background: '#fff', borderRadius: 12, padding: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid #E2E8F0', boxShadow: '0 2px 12px rgba(10,31,68,0.08)' }}>
            {visit.qrToken ? <QrCodeImage token={visit.qrToken} /> : <div style={{ textAlign: 'center' }}><div style={{ fontSize: '1.2rem' }}>🔲</div><div style={{ fontSize: '0.4rem', color: '#888', textTransform: 'uppercase', letterSpacing: '0.1em' }}>No QR</div></div>}
          </div>
          ...
          {!past && (
            <div style={{ marginTop: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid rgba(22,163,74,0.3)', background: 'rgba(22,163,74,0.06)', fontSize: '0.5rem', fontWeight: 800, color: '#16a34a', textTransform: 'uppercase', letterSpacing: '0.1em', textAlign: 'center' }}>
              Active — No Divert
            </div>
          )}
        </div>
      ...
      <div style={{ background: '#F8FAFC', borderTop: '1px solid #E2E8F0', padding: '10px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: '0.5rem', color: '#6B7FA3', letterSpacing: '0.2em', textTransform: 'uppercase', fontWeight: 800 }}>Approved {fmtDate(vd)}</div>
...
// ─── Incoming Divert Requests Panel ──────────────────────────────────────────
interface IncomingDivertPanelProps { onRefresh: () => void; }
function IncomingDivertPanel({ onRefresh }: IncomingDivertPanelProps) {
  ...
  const fetchRequests = async () => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/api/v1/visits/divert-requests/incoming`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => {
    fetchRequests();
    socket.on('divert_request_received', fetchRequests);
    return () => { socket.off('divert_request_received', fetchRequests); };
  }, []);

  const respond = async (visitId: string, reqId: string, action: 'accept' | 'reject') => {
    setResponding(reqId);
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${API_URL}/api/v1/visits/${visitId}/divert-request/${reqId}/respond`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action }),
      });
      if (res.ok) { fetchRequests(); onRefresh(); }
    } catch {} finally { setResponding(null); }
  };

  const myUserId = (() => { try { return JSON.parse(localStorage.getItem('user') || '{}')._id; } catch { return ''; } })();

  const pendingItems = requests.flatMap(visit =>
    (visit.divertRequests || [])
      .filter(dr => dr.status === 'Pending' && dr.to && (typeof dr.to === 'string' ? dr.to : (dr.to as any)._id) === myUserId)
      .map(dr => ({ visit, dr }))
  );
  ...
  return (
    <div style={{ marginBottom: 28, padding: '20px 24px', background: 'linear-gradient(135deg, rgba(47,93,170,0.04), rgba(124,58,237,0.04))', border: '1.5px solid rgba(47,93,170,0.2)', borderRadius: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#2F5DAA', animation: 'pulse 2s ease-in-out infinite' }}/>
        <span style={{ fontSize: '0.65rem', fontWeight: 900, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#0A1F44' }}>Incoming Divert Requests</span>
        <span style={{ background: '#2F5DAA', color: '#fff', borderRadius: 9999, padding: '1px 8px', fontSize: '0.6rem', fontWeight: 900 }}>{pendingItems.length}</span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {pendingItems.map(({ visit, dr }) => (
          <div key={dr._id} style={{ background: '#fff', borderRadius: 14, border: '1px solid #E2E8F0', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(47,93,170,0.08)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.85rem', fontWeight: 800, color: '#2F5DAA', overflow: 'hidden' }}>
              {visit.visitor?.imageUrl
                ? <img src={visit.visitor.imageUrl.startsWith('data:') ? visit.visitor.imageUrl : `${API_URL}${visit.visitor.imageUrl}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt=""/>
                : visit.visitor?.name?.charAt(0)
              }
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#0A1F44' }}>{visit.visitor?.name}</div>
              <div style={{ fontSize: '0.65rem', color: '#6B7FA3', marginTop: 2 }}>
                <strong style={{ color: '#2F5DAA' }}>{(dr.from as any)?.name || 'A colleague'}</strong> wants to transfer this visit to you
                {dr.reason ? <span style={{ color: '#94A3B8' }}> · "{dr.reason}"</span> : ''}
              </div>
              <div style={{ fontSize: '0.6rem', color: '#94A3B8', marginTop: 2 }}>Purpose: {visit.purpose}</div>
            </div>
            <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
              <button
                onClick={() => respond(visit._id, dr._id, 'reject')}
                disabled={responding === dr._id}
                style={{ padding: '7px 14px', borderRadius: 9, border: '1.5px solid rgba(239,68,68,0.3)', background: 'rgba(239,68,68,0.05)', fontSize: '0.65rem', fontWeight: 800, color: '#ef4444', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.1em' }}
              >Decline</button>
              <button
                onClick={() => respond(visit._id, dr._id, 'accept')}
                disabled={responding === dr._id}
                style={{ padding: '7px 16px', borderRadius: 9, border: 'none', background: '#0A1F44', fontSize: '0.65rem', fontWeight: 800, color: '#fff', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.1em' }}
              >{responding === dr._id ? '…' : '✓ Accept'}</button>
            </div>
          </div>
        ))}
      </div>
...
function matchesSearch(v: Visit, q: string) {
  ...
  return !!(v.visitor?.name?.toLowerCase().includes(s) || v.visitor?.phone?.includes(s) || v.meetWith?.name?.toLowerCase().includes(s) || v.meetWithDept?.name?.toLowerCase().includes(s) || v.purpose?.toLowerCase().includes(s));
}
...
export default function DashboardApprovals() {
  ...
  useEffect(() => {
    ...
    socket.on('divert_request_responded', fetchPending);
    return () => { socket.off('new_visit', fetchPending); socket.off('new_visit_request', fetchPending); socket.off('approval_updates'); socket.off('divert_request_responded', fetchPending); };
  }, []);

  useEffect(() => { if (tab === 'approved' && approvedVisits.length === 0) fetchApproved(); }, [tab]);
  ...
  return (
    <div className="fade-up w-full">
      {/* Divert request modal — only for Pending visits */}
      {divertVisit && (
        <DivertRequestModal
          visit={divertVisit}
          myDeptId={myDeptId}
      ...
      {/* Incoming divert requests panel */}
      <IncomingDivertPanel onRefresh={() => { fetchPending(); fetchApproved(); }} />
              ...
              {pendingVisits.map((visit) => (
                <div key={visit._id} className="vp-card p-8 relative overflow-hidden">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    {/* Show "Divert Requested" badge if there's a pending divert request out */}
                    {visit.divertRequests?.some(dr => dr.status === 'Pending') && (
                      <span style={{ marginLeft: 4, background: 'rgba(47,93,170,0.08)', color: '#2F5DAA', borderRadius: 6, padding: '2px 8px', fontSize: '0.55rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                        Divert Pending…
                      </span>
                    )}
                  </div>
                  ...
                  {/* Divert request button — only on Pending visits, only for employees with a dept */}
                  {myDeptId && !visit.divertRequests?.some(dr => dr.status === 'Pending') && (
                    <button onClick={() => setDivertVisit(visit)}
                      style={{ marginTop: 10, width: '100%', padding: '9px', borderRadius: 12, border: '1.5px solid rgba(47,93,170,0.2)', background: 'rgba(47,93,170,0.04)', fontSize: '0.65rem', fontWeight: 800, color: '#2F5DAA', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.15em' }}>
          ...
          : (
            <div>
              <div style={{ display: 'flex', gap: 10, marginBottom: 28, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                {hasFilters && <button onClick={() => { setSearch(''); setDateFrom(''); setDateTo(''); }} style={{ padding: '9px 16px', borderRadius: 12, border: '1.5px solid #E2E8F0', background: '#F8FAFC', fontSize: '0.7rem', fontWeight: 800, color: '#6B7FA3', cursor: 'pointer', textTransform: 'uppercase' }}>Clear</button>}
              </div>

              {upcoming.length === 0 ? (
                <div className="py-14 text-center border border-[#E2E8F0] bg-[#F8FAFC] rounded-3xl mb-8">
                  <div style={{ fontSize: '2rem', marginBottom: 10 }}>📅</div>
                  <p className="text-[#6B7FA3] text-sm">{hasFilters ? 'No results match your search.' : 'No upcoming approved visits.'}</p>
                </div>
              ) : (
                <div style={{ marginBottom: 36 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#22C55E' }}/>
                    <span style={{ fontSize: '0.65rem', fontWeight: 900, letterSpacing: '0.25em', textTransform: 'uppercase', color: '#0A1F44' }}>Active — {upcoming.length} pass{upcoming.length !== 1 ? 'es' : ''}</span>
                    <span style={{ fontSize: '0.6rem', color: '#94A3B8', fontStyle: 'italic' }}>Active visits cannot be diverted</span>
                  </div>
              ...
              {past.length > 0 && (
                <div>
                  <button onClick={() => setPastOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '14px 20px', borderRadius: 16, border: '1.5px solid #E2E8F0', background: '#F8FAFC', cursor: 'pointer', marginBottom: pastOpen ? 20 : 0 }}>
                    <svg style={{ width: 14, height: 14, color: '#6B7FA3', transition: 'transform 0.2s', transform: pastOpen ? 'rotate(90deg)' : 'none' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 5l7 7-7 7"/></svg>
