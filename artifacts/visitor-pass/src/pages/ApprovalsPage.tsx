...
export default function ApprovalsPage() {
  ...
  const fetchVisits = async (t: string) => {
    try {
      const res = await fetch(`${API_URL}/api/v1/visits/pending`, {
      ...
      <header className="bg-white border-b border-[#E2E8F0] px-8 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { localStorage.clear(); navigate('/login'); }}
            className="text-xs font-semibold text-[#6B7FA3] hover:text-[#0A1F44] transition-colors">Sign Out</button>
        </div>
      </header>
            ...
            {visits.map(visit => (
              <div key={visit._id} className="vp-card p-6">
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <div className="ml-auto"><span className="badge badge-pending">Pending</span></div>
                </div>
