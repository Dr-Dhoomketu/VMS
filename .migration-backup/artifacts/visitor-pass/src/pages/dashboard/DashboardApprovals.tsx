...
function VisitorPass({ visit }: { visit: Visit }) {
  const visitorId = `VMS-${visit._id?.slice(-6).toUpperCase()}`;
  const approvedDate = visit.updatedAt ? new Date(visit.updatedAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
  const approvedTime = visit.updatedAt ? new Date(visit.updatedAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : '—';
  return (
    <div style={{ background:'linear-gradient(135deg,#ffffff 0%,#f8fafc 50%,#ffffff 100%)', border:'1px solid #E2E8F0', borderRadius:24, overflow:'hidden', boxShadow:'0 4px 24px rgba(10,31,68,0.08)', position:'relative' }}>
      ...
      <div style={{ padding:24, display:'flex', gap:20, background:'#ffffff' }}>
        <div style={{ flexShrink:0 }}>
          ...
          <div style={{ marginTop:8, textAlign:'center', background:'rgba(34,197,94,0.1)', border:'1px solid rgba(34,197,94,0.3)', borderRadius:6, padding:'3px 0', fontSize:'0.5rem', fontWeight:900, letterSpacing:'0.2em', textTransform:'uppercase', color:'#16a34a' }}>✓ APPROVED</div>
        </div>
      ...
      <div style={{ background:'#F8FAFC', borderTop:'1px solid #E2E8F0', padding:'10px 24px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ fontSize:'0.5rem', color:'#6B7FA3', letterSpacing:'0.2em', textTransform:'uppercase', fontWeight:800 }}>Approved {approvedDate} at {approvedTime}</div>
  ...
  return (
    <div className="fade-up w-full">
      ...
      <div className="flex gap-1 mb-8 bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-1 w-fit">
        {[{ key:'pending', label:'Pending', count:pendingVisits.length },{ key:'approved', label:'Approved', count:approvedVisits.length }].map(t => (
      ...
      {tab === 'approved' && (
        approvedLoading ? (
          ...
          <div className="py-20 text-center border border-[#E2E8F0] bg-[#F8FAFC] rounded-3xl"><p className="text-[#6B7FA3] text-sm">No approved visits yet.</p></div>
        ) : (
          <div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">{approvedVisits.map(v => <VisitorPass key={v._id} visit={v}/>)}</div>
          </div>
        )
      )}
    </div>
