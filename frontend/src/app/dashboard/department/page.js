...
export default function DepartmentManagement() {
  ...
  // FIXED: Initializing with empty strings to prevent "uncontrolled to controlled" error
  ...
  return (
    <div className="fade-up w-full">
      <div className="flex justify-between items-center mb-12">
        <div>
          <h1 className="text-4xl font-black tracking-tighter uppercase mb-2 text-[#0A1F44]">Departments</h1>
          <p className="text-[#6B7FA3] text-sm">Manage organizational structure and units.</p>
        </div>
        <button 
        ...
        <table className="dark-table w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="3" className="py-12 text-center text-gray-500 italic">No departments recorded.</td></tr>
            ) : (
              departments.map((dept) => (
                <tr key={dept._id} className="group hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4">
                    <span className="text-[#0A1F44] font-bold uppercase tracking-wide">{dept.name}</span>
                  </td>
                  <td className="px-6 py-4 text-[#6B7FA3] text-sm font-mono">{dept.code}</td>
                  <td className="px-6 py-4 text-center">
                    <div className="flex justify-center gap-3">
                      <button onClick={() => handleOpenModal(dept)} className="text-[#6B7FA3] hover:text-[#0A1F44] transition-colors">
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                      </button>
                      <button onClick={() => handleDelete(dept._id)} className="text-[#6B7FA3] hover:text-red-500 transition-colors">
      ...
      <Modal 
        ...
        title={currentDept ? 'Edit Department' : 'Create Department'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Department Name</label>
            <input required type="text" className="w-full py-2" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="e.g. Engineering" />
          </div>
          <div>
            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Department Code</label>
            <input required type="text" className="w-full py-2" value={formData.code} onChange={e => setFormData({...formData, code: e.target.value})} placeholder="e.g. ENG" />
          </div>
          <div className="pt-2">
            <button type="submit" className="w-full btn-primary py-3.5 rounded-xl font-black tracking-widest uppercase">
              {currentDept ? 'Save Changes' : 'Confirm Department'}
            </button>
          </div>
        </form>
      </Modal>
